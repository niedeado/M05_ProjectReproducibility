...
def visualize_report(y_true, y_pred, labels_inv_map):
    ...
    labels_true, labels_predict, labels_order = get_labels_analysis(
...
def inspect_misclassified(
    y_true, y_pred, labels_inv_map, iter_bound=ITER_BOUND, print_bound=PRINT_BOUND
):
    ...
    while (print_count < print_bound) and (i < iter_bound):
        ...
        if idxs_cm[0][i] != idxs_cm[1][i]:
            print_count += 1
            misclassified_msg.append(
                f"{labels_inv_map[idxs_cm[0][i]]} was predicted as {labels_inv_map[idxs_cm[1][i]]}: {cm[idxs_cm[0][i], idxs_cm[1][i]]} times"
            )
