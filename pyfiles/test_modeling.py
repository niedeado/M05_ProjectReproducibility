...
def test_extract_data_array(dataset_setup):
    ...
    assert np.equal(X, np.array([[0.5, 0.2],
                                 [0.6, 0.1],
                                 [0.3, 0.7],
                                 [0.8, 0.3]])).all()
...
def test_get_labels_analysis():
    ...
    labels_true, labels_predict, labels_order = analysis.get_labels_analysis(y_true, y_pred, labels_inv_map)
    assert labels_true == ['Pluto', 'Pluto', 'Pippo', 'Pippo', 'Pluto', 'Pippo', 'Pippo']
    assert labels_predict == ['Pluto', 'Pluto', 'Pluto', 'Pippo', 'Pippo', 'Pluto', 'Pippo']
...
def test_inspect_misclassified():
    ...
    assert misclassified_msg == ['Pippo was predicted as Pluto: 2 times', 'Pluto was predicted as Pippo: 1 times']
...
def test_selection_criteria():
    parameters = [{'n_estimators': 3, 'max_depth': None, 'max_features': 'auto'},
